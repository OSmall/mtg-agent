...
import {
    DeckBuildingBriefSchema,
    type DeckCandidate,
    type DeckCandidateRepository,
    type DeckCandidateRepositoryError,
    DeckCandidateSchema,
    type DeckCandidateSummary
} from "@tomekin/core";
  ...
  return {
    async saveDeckCandidate(input) {
      return wrap(() => {
        ...
        db.transaction((tx) => {
          if (existing) {
              tx.update(deckCandidate).set({
                  label: input.label,
                  format: input.brief.format,
                  formatAnchor: input.formatAnchor,
                  briefJson: input.brief,
                  collectionImportTimestamp: input.collectionImportTimestamp,
                  markdown: input.markdown,
                  updatedAt: now
              }).where(eq(deckCandidate.id, id)).run();
            tx.delete(deckCandidateCard).where(eq(deckCandidateCard.deckCandidateId, id)).run();
          } else {
              tx.insert(deckCandidate).values({
                  id,
                  label: input.label,
                  format: input.brief.format,
                  formatAnchor: input.formatAnchor,
    ...
    async listDeckCandidates() {
        return wrap(() => db.select({
            id: deckCandidate.id,
            label: deckCandidate.label,
            format: deckCandidate.format,
            formatAnchor: deckCandidate.formatAnchor,
            collectionImportTimestamp: deckCandidate.collectionImportTimestamp,
            createdAt: deckCandidate.createdAt,
            updatedAt: deckCandidate.updatedAt,
            cardCount: sql<number>`coalesce(sum(
            ${deckCandidateCard.quantity}
            ),
            0
            )`
        }).from(deckCandidate).leftJoin(deckCandidateCard, eq(deckCandidate.id, deckCandidateCard.deckCandidateId)).groupBy(deckCandidate.id).orderBy(asc(deckCandidate.label)).all().map((row): DeckCandidateSummary => ({
            ...row,
...
function readCandidate(db: TomekinDatabase, id: string): DeckCandidate {
  ...
  const cards = db.select({cardIdentityId: deckCandidateCard.cardIdentityId, quantity: deckCandidateCard.quantity, section: deckCandidateCard.section, sortOrder: deckCandidateCard.sortOrder, note: deckCandidateCard.note, cardName: cardIdentity.name}).from(deckCandidateCard).innerJoin(cardIdentity, eq(deckCandidateCard.cardIdentityId, cardIdentity.id)).where(eq(deckCandidateCard.deckCandidateId, id)).orderBy(asc(deckCandidateCard.sortOrder), asc(cardIdentity.name)).all();
    const {briefJson, ...candidate} = row;
    return DeckCandidateSchema.parse({...candidate, brief: DeckBuildingBriefSchema.parse(briefJson), cards});
}
